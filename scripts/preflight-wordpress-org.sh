...
PLUGIN_SLUG="${PLUGIN_SLUG:-simple-link-card}"
VERSION="${VERSION:-$(sed -n 's/^ \* Version: //p' "$ROOT_DIR/simple-link-card.php" | head -n 1)}"
RELEASE_DIR="${SLC_RELEASE_DIR:-/tmp/${PLUGIN_SLUG}-release}"
ZIP_PATH="${SLC_ZIP_PATH:-/tmp/${PLUGIN_SLUG}-${VERSION}.zip}"
PHPCS_BIN="${SLC_PHPCS_BIN:-/Users/tadashi/local/develop/app/public/wp-content/plugins/plugin-check/vendor/squizlabs/php_codesniffer/bin/phpcs}"
...
if [ ! -f "$PHPCS_BIN" ]; then
  ...
  echo "Set SLC_PHPCS_BIN to your phpcs path." >&2
  exit 1
fi

if [ -z "$VERSION" ]; then
  echo "Could not detect plugin version from simple-link-card.php." >&2
  exit 1
fi
