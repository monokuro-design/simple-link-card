...
class Renderer {
    ...
    private function generate_html( $ogp_data, $image_position = 'left', $show_site_name = true, $show_description = true, $open_in_new_tab = true ) {
        ...
        // カードのクラス名を生成
        $card_classes = array( 'slc-card' );
        if ( $image_position === 'right' && ! empty( $image ) ) {
            $card_classes[] = 'slc-card--image-right';
        } elseif ( $image_position === 'hide' || empty( $image ) ) {
            $card_classes[] = 'slc-card--no-image';
        }

        $html = sprintf(
            '<div class="wp-block-simple-link-card" data-url="%s">',
            esc_attr( $url )
        );
        ...
        if ( ! empty( $image ) && $image_position !== 'hide' ) {
            $html .= sprintf(
                '<div class="slc-card__image"><img src="%s" alt="%s" loading="lazy" /></div>',
                $image,
                esc_attr( $title )
            );
        }
        ...
        // コンテンツ部分
        $html .= '<div class="slc-card__content">';

        // タイトル
        $html .= sprintf( '<div class="slc-card__title">%s</div>', $title );

        // 説明（表示設定がオンの場合のみ）
        if ( $show_description && ! empty( $description ) ) {
            $html .= sprintf( '<p class="slc-card__description">%s</p>', $description );
        }

        // サイト情報（表示設定がオンの場合のみ）
        if ( $show_site_name && ( ! empty( $favicon ) || ! empty( $display_site ) ) ) {
            $html .= '<div class="slc-card__site">';
            if ( ! empty( $favicon ) ) {
                $html .= sprintf(
                    '<img class="slc-card__favicon" src="%s" alt="" loading="lazy" />',
                    $favicon
                );
            }
            if ( ! empty( $display_site ) ) {
                $html .= sprintf( '<span class="slc-card__domain">%s</span>', $display_site );
            }
            $html .= '</div>';
        }

        $html .= '</div>'; // .slc-card__content

        $html .= '</a>'; // .slc-card
        $html .= '</div>'; // .wp-block-simple-link-card
         ...
         */
        return apply_filters( 'simple_link_card_card_html', $html, $ogp_data, array(
    ...
    public function render_placeholder() {
        return '<div class="wp-block-simple-link-card slc-placeholder">' .
               '<p>' . esc_html__( 'URLを入力、または投稿を検索してください', 'simple-link-card' ) . '</p>' .
               '</div>';
    }
    ...
    public function render_error( $message ) {
        return '<div class="wp-block-simple-link-card slc-error">' .
    ...
    public function render_loading() {
        return '<div class="wp-block-simple-link-card slc-loading">' .
               '<div class="slc-spinner"></div>' .
               '<p>' . esc_html__( 'OGP情報を取得中...', 'simple-link-card' ) . '</p>' .
               '</div>';
    }
}
