...
class REST_API {
     ...
     * @var string
     */
    private $namespace = 'simple-link-card/v1';
    ...
    public function check_permission() {
        ...
        if ( ! current_user_can( 'publish_posts' ) ) {
            return new \WP_Error(
                'rest_forbidden',
                __( 'OGP情報を取得する権限がありません。', 'simple-link-card' ),
                array( 'status' => 403 )
            );
        }
        return true;
    ...
    private function check_rate_limit( $user_id ) {
        $transient_key = 'slc_rate_limit_' . $user_id;
        ...
        if ( $requests >= 30 ) {
            return new \WP_Error(
                'rate_limit_exceeded',
                __( 'リクエスト数の上限を超えました。しばらく待ってから再度お試しください。', 'simple-link-card' ),
                array( 'status' => 429 )
            );
        }
    ...
    public function check_admin_permission() {
        if ( ! current_user_can( 'manage_options' ) ) {
            return new \WP_Error(
                'rest_forbidden',
                __( 'この操作を行う権限がありません。', 'simple-link-card' ),
                array( 'status' => 403 )
            );
        }
        return true;
    ...
    public function clear_cache( $request ) {
        ...
        if ( empty( $url ) ) {
            return new \WP_Error(
                'missing_url',
                __( 'URLが指定されていません。', 'simple-link-card' ),
                array( 'status' => 400 )
            );
        }
        ...
        return new \WP_REST_Response( array(
            ...
            'message' => sprintf( __( '%s のキャッシュをクリアしました。', 'simple-link-card' ), $url ),
            'url'     => $url,
        ), 200 );
    }
